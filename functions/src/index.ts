...
export const reviewSubmission = onCall(async (request) => {
  ...
  if (!DATASET_COLLECTIONS.has(collection) || !recordId || !["APPROVED", "REJECTED"].includes(decision)) throw new HttpsError("invalid-argument", "Invalid review request.");
  ...
  await ref.update({ status: decision, reviewedBy: request.auth.uid, reviewedAt: FieldValue.serverTimestamp(), reviewNotes: notes, updatedAt: FieldValue.serverTimestamp() });
...
export const transitionDataStage = onCall(async (request) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    tx.update(ref, { dataStage: targetStage, stage: targetStage, stageChangedBy: actorUid, stageChangedAt: now, stageComments: comments, stageConfidenceScore: confidenceScore || null, updatedAt: now });
    tx.set(db.collection("auditLogs").doc(), { action: "DATA_STAGE_TRANSITION", actorUid, collection, recordId, previousStage: currentStage, newStage: targetStage, comments, confidenceScore: confidenceScore || null, createdAt: now });
  });
...
export const voteOnCommunityPost = onCall(async (request) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (vote.exists) { tx.delete(voteRef); tx.update(postRef, { voteScore: FieldValue.increment(-1), updatedAt: FieldValue.serverTimestamp() }); }
    else { tx.set(voteRef, { createdAt: FieldValue.serverTimestamp() }); tx.update(postRef, { voteScore: FieldValue.increment(1), updatedAt: FieldValue.serverTimestamp() }); }
  });
