...
class KhowarViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // No automatic/demo identity is created. Authentication must be explicit.
    ...
    /** Users cannot elevate or change their own role from the client. */
    fun switchUserRole(role: UserRole) {
        _statusMessage.value = "Role switching is disabled. Sign in with an authorized account."
    }

    /** Role management is restricted to administrators; SUPER_ADMIN may only be granted by SUPER_ADMIN. */
