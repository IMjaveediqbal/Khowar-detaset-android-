...
object RbacPolicy {
    private val permissionsByRole: Map<UserRole, Set<Permission>> = mapOf(
        ...
        UserRole.EXPERT to setOf(Permission.READ_PUBLIC_DATASET, Permission.VALIDATE_COMMUNITY, Permission.EXPERT_VERIFY, Permission.EXPORT_RESEARCH_DATA, Permission.VIEW_AUDIT_LOGS),
        ...
        UserRole.DATA_STEWARD to setOf(Permission.READ_PUBLIC_DATASET, Permission.VALIDATE_COMMUNITY, Permission.MANAGE_METADATA, Permission.MANAGE_DATA_STEWARDSHIP, Permission.EXPERT_VERIFY, Permission.EXPORT_RESEARCH_DATA, Permission.VIEW_AUDIT_LOGS),
...
@Entity(tableName = "lexicon_entries", indices = [Index(value = ["status", "createdAt"]), Index(value = ["contributorId"])])
...
@Entity(tableName = "sentences", indices = [Index(value = ["status", "createdAt"]), Index(value = ["contributorId"])])
...
@Entity(tableName = "speech_recordings", indices = [Index(value = ["status", "createdAt"]), Index(value = ["contributorId"])])
data class SpeechRecording(@PrimaryKey val id: String = UUID.randomUUID().toString(), val speakerPublicId: String, val speakerAgeGroup: String, val speakerGender: String, val isNativeSpeaker: Boolean = true, val audioFilePath: String, val durationSeconds: Double, val sampleRate: Int = 44100, val channels: Int = 1, val format: String = "M4A/AAC", val transcriptKhowar: String, val normalizedTranscript: String, val transliteration: String, val englishTranslation: String, val urduTranslation: String, val dialectId: String = "Central", val regionId: String = "Chitral", val recordingEnvironment: String = "Quiet room", val qualityScore: Double = 0.0, val contributorId: String, val contributorName: String, val status: RecordStatus = RecordStatus.SUBMITTED, val licenseId: String = "CC-BY-SA-4.0", val dataStage: DataStage = DataStage.RAW, val datasetVersion: String? = null, val createdAt: Long = System.currentTimeMillis())
@Entity(tableName = "stories", indices = [Index(value = ["status", "createdAt"]), Index(value = ["contributorId"])])
...
@Entity(tableName = "images", indices = [Index(value = ["status", "createdAt"]), Index(value = ["contributorId"])])
...
@Entity(tableName = "knowledge", indices = [Index(value = ["status", "createdAt"]), Index(value = ["contributorId"])])
