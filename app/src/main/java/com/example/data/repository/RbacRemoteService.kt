...
class RbacRemoteService(
    ...
    suspend fun getMyRbac(): Result<Pair<String, String>> = runCatching {
        ...
        val data = result.data as? Map<*, *> ?: error("Invalid RBAC response.")
    ...
    suspend fun setUserRole(targetUid: String, role: String, reason: String): Result<Unit> = runCatching {
        ...
        functions.getHttpsCallable("setUserRole").call(mapOf("targetUid" to targetUid, "role" to role.uppercase(), "reason" to reason.trim())).await()
        Unit
    }

    suspend fun completeManagedPasswordChange(): Result<Unit> = runCatching {
        functions.getHttpsCallable("completeManagedPasswordChange").call().await()
        Unit
    }
    ...
    suspend fun transitionDataStage(collection: String, recordId: String, targetStage: String, comments: String = "", confidenceScore: Int = 0): Result<Unit> = runCatching {
        ...
        functions.getHttpsCallable("transitionDataStage").call(mapOf("collection" to collection.lowercase(), "recordId" to recordId, "targetStage" to targetStage.uppercase(), "comments" to comments, "confidenceScore" to confidenceScore)).await()
        Unit
    }
}
